...
class Ant(Insect):
    ...
    def remove_from(self, place):
        if place.ant is self:
            place.ant = None
        elif place.ant is None:  # If we try to remove an ant from a place when it's not assigned yet
            assert False, '{0} is not in {1}'.format(self, place)
        else:   # If the passed place is not allowed to store ants
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        position_to_check = self.place
        counter = 0
        while not (position_to_check.bees and self.lower_bound <= counter <= self.upper_bound):
            position_to_check = position_to_check.entrance
            counter += 1
            if position_to_check.is_hive:
                return None
        if self.lower_bound <= counter <= self.upper_bound:
            return random.choice(position_to_check.bees)
        else:
            return None
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        for bee in self.place.bees[:]:
            bee.reduce_health(amount + self.damage*sum([amount>=self.health]))
...
class ContainerAnt(Ant):
    ...
    def remove_from(self, place):
        ...
        if place.ant is self:
            ...
            Ant.remove_from(self, place) #my source code change (Ant-->Insect)

    def action(self, gamestate):
        # BEGIN Problem 8a
        if self.ant_contained:
            self.ant_contained.action(gamestate)
            Ant.action(self.ant_contained, gamestate)
        Ant.action(self, gamestate)
        # END Problem 8a
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        # BEGIN Problem 13
        if target is not None:
            target.turns_to_move = self.slow_turns

            def slow_action(gamestate):
                if target.turns_to_move > 0:
                    target.turns_to_move -= 1
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        while not position.is_hive:
            if position.ant and position.ant is not self:
                insect_dict.update({position.ant:dist_counter})
                if position.ant.is_container and position.ant.ant_contained and position.ant.ant_contained is not self:
                    insect_dict.update({position.ant.ant_contained:dist_counter})
            for bee in position.bees:
                insect_dict.update({bee:dist_counter})
            dist_counter += 1
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        calculated = (self.damage * ((3/4) ** (distance))) - ((0.0625) * self.insects_shot)
        if calculated < 0:
            return 0
